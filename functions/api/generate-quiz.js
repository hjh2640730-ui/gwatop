...
L8. 【이미지 참조 문제 — 매우 중요】
   ...
   ★ 절대 금지: 이미지에서 값·수치·레이블을 그대로 읽으면 답이 나오는 문제
     예) "그래프에서 A 시점의 값은?" → 금지 (이미지 보면 바로 답 보임)
         "표의 3행 2열에 해당하는 값은?" → 금지
         "그림에서 ㉠에 들어갈 말은?" → 금지 (이미지에 글자가 적혀 있으면)

   ★ 이미지 참조 문제는 반드시 이 조건을 충족해야 함:
     · 해당 분야의 개념적 이해가 없으면, 이미지를 봐도 답을 알 수 없어야 함
     · 이미지는 '문제의 맥락·데이터'를 제공할 뿐, 답 자체가 이미지에 보여선 안 됨
     · 좋은 예:
       "위 그래프에서 나타난 추세의 원인으로 가장 적절한 것은?" (해석 필요)
       "위 실험 결과에서 대조군 대비 실험군의 차이가 의미하는 바는?" (분석 필요)
       "위 도표의 경향이 지속될 경우 예측되는 결과는?" (추론 필요)
       "위 그림에서 A 과정이 일어나는 조건으로 옳은 것은?" (개념 적용 필요)` : ''}
